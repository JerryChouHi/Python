...
class Hero(Card):
    def __init__(self, hero):
        ...
        if self.hero_class == 'shaman':
            ...
            airfury_totem = Card('空气之怒图腾', 'totem', 'None', 0, 2, 1)
            heal_totom = Card('治疗图腾', 'totem', 'round_begin', 0, 2, 1)
            hot_totem = Card('灼热图腾', 'totem', 'None', 1, 1, 1)
            self.totem = [stone_totem, airfury_totem, heal_totom, hot_totem]


def show_cards(cards):
    for i, card in enumerate(cards):
        print("\t{0}-{1}({2})".format(i + 1, card.name, card.mana))
...
def choose_attack_num(attack_list, desc):
    ...
    if attack_num == 0:
        ...
        if attack_list[attack_num - 1].property == 'Taunt':
            return attack_num
        else:
            print("必须攻击具有嘲讽属性的卡牌！")
        ...
        if self.myturn:
            if order:
                ...
                for i in range(len(self.enemyworldcards)):
                    ...
                    if self.enemyworldcards[i].property == 'Taunt':
                        print("[嘲讽]", end=' ')
                    else:
                        print(end=' ')
            else:
        ...
        if self.myturn:
            ...
            for i in range(len(mysite) - 1, -1, -1):
                ...
                if i == 0 and self.myhero.damage > 0:
                    ...
                    if attack_num == 0:
                        continue
                    self.myhero.attack(opposite[attack_num - 1])
                    ...
                    if mysite[i].damage > 0 and mysite[i].sleep == False:
                        ...
                        if attack_num == 0:
                            continue
                        mysite[i].attack(opposite[attack_num - 1])
            ...
            for i in range(len(enemysite) - 1, -1, -1):
                ...
                if i == 0 and self.enemyhero.damage > 0:
                    ...
                    if enemysite[i].damage > 0 and enemysite[i].sleep == False:
                        ...
                        if opposite[rand_num].armor > 0:
                            print("[{0}]".format(opposite[rand_num].armor), end='')
                        if opposite[rand_num].property == 'Taunt':
                            print("[嘲讽]")
                        else:
                            print('')
                        if rand_num == 0:
    ...
    def use_mana_action(self, mana):
        ...
        if self.myturn:
            while self.can_use_skill(mana) or (
                ...
                if self.can_use_skill(mana) and len(self.myworldcards) < WORLDCARDNUM:
    ...
    def attack_who(self, cards):
        ...
        if len(cards) == 1:
            ...
            if card_has_taunt(cards):
                if cards[card_id].property == 'Taunt':
                    return card_id
                else:
                    return self.attack_who(cards)
            else:
