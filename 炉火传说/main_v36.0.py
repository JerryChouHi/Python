...
class World:
    ...
    def round_begin(self):
        ...
        if self.myturn:
            ...
            for card in self.myworldcards:
                if card.present_property == 'MyRoundBegin':
                    print("------> 回合开始触发 <------")
                    if card.name == '治疗图腾':
    ...
    def update_world(self):
        ...
        for i in range(len(self.myworldcards) - 1, -1, -1):
            ...
            if card.remain_health <= 0 or not card.alive:
                self.mydeadthfollower.append(card)
                for handcard in self.myhandcards:
        ...
        for i in range(len(self.enemyworldcards) - 1, -1, -1):
            ...
            if card.remain_health <= 0 or not card.alive:
                self.enemydeathfollower.append(card)
                for handcard in self.myhandcards:
                    if handcard.name == '通道爬行者' and handcard.present_mana > 0:
                        handcard.present_mana -= 1
                for handcard in self.enemyhandcards:
    ...
    def battle_cry(self, card, myhero, oppohero, myworldcards, oppoworldcards, myhandcards, oppohandcards,
        ...
        if card.name == '死亡之翼':
            ...
            if card.name == '火羽精灵':
                ...
                if self.myturn:
                    deathrattle_list = Deathrattle_cards(self.mydeadthfollower)
    ...
    def auto_goto_world(self):
        ...
        while index != -2:
            for i in range(len(self.enemyhandcards)):
                if self.enemyhandcards[i].present_mana <= self.current_mana:
                    select = self.current_mana - self.enemyhandcards[i].present_mana
                    index = i
                    break
                else:
                    index = -2
            if select > 0 and -1 < index < len(self.enemyhandcards) - 1:
                for j in range(index + 1, len(self.enemyhandcards)):
                    if self.enemyhandcards[j].present_mana <= self.current_mana:
                        select2 = self.current_mana - self.enemyhandcards[j].present_mana
                        if select > select2:
                            select = select2
                            index = j
