...
class Hero(Card):
    def __init__(self, hero):
        ...
        if self.hero_class == 'Shaman':
            ...
            heal_totom = Card('', '治疗图腾', 'totem', 'RoundBegin', 0, 2, 1, '', 'Follower')
...
class World:
    ...
    def round_end(self):
        ...
        if self.myturn:
            for card in self.myworldcards:
                if card.property == 'RoundEnd':
                    print("------> 回合结束触发 <------")
                    if card.name == '迦顿男爵':
                        for worldcard in self.myworldcards:
                            if worldcard.name!='迦顿男爵':
                                print("{0} 的生命值 {1}->".format(worldcard.name,worldcard.remain_health),end='')
                        ...
                        for worldcard in self.enemyworldcards:
                            ...
                            worldcard.reduce_health(2)
                            print("{0}".format(worldcard.remain_health))
                        print("{0} 的生命值 {1}->".format(self.myhero.name, self.myhero.remain_health), end='')
                        self.myhero.reduce_health(2)
                        print("{0}".format(self.myhero.remain_health))
                        print("{0} 的生命值 {1}->".format(self.enemyhero.name, self.enemyhero.remain_health), end='')
    ...
    def skill_using(self):
        ...
        if self.myturn:
            ...
            if self.myhero.hero_class == 'Warrior':
                ...
                if not world[choose_num - 1].alive:
                    for card in self.myhandcards:
                        if card.name == '通道爬行者' and card.mana > 0:
                            card.mana -= 1
                    for card in self.enemyhandcards:
                        if card.name == '通道爬行者' and card.mana > 0:
                            card.mana -= 1
