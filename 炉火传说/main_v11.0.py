...
class world:
    ...
    def draw_card(self):
        ...
        if self.init_hand_count < CARD_NUM:
            if self.myturn and len(self.myhandcards)<HANDCARDNUM:
                self.myhandcards.append(self.mycards[self.init_hand_count])
            elif self.myturn and len(self.myhandcards)==HANDCARDNUM:
                print("我方手牌已满，【{0}】被摧毁！".format(self.mycards[self.init_hand_count].name))
            elif self.myturn==False and len(self.enemyhandcards)<HANDCARDNUM:
                self.enemyhandcards.append(self.enemycards[self.init_hand_count])
            elif self.myturn==False and len(self.enemyhandcards)==HANDCARDNUM:
                print("敌方手牌已满，【{0}】被摧毁！".format(self.mycards[self.init_hand_count].name))
        else:
            if self.myturn:
    ...
    def use_mana_action(self, mana):
        ...
        """
        if self.myturn:
            while self.can_use_skill(mana) or (self.enough_mana(self.myhandcards, mana) and len(self.myworldcards)<WORLDCARDNUM):
                print("^^^^我方手牌^^^^^^^^^^^^")
                ...
                if self.can_use_skill(mana):
                    ...
                    elif self.enough_mana(self.myhandcards, mana) and len(self.myworldcards)<WORLDCARDNUM:
