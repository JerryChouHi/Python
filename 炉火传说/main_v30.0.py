...
class World:
    ...
    def hero_can_attack(self):
        ...
        can_attack = False
        if self.myturn:
            if self.myhero.can_attack and self.myhero.present_damage > 0:
                can_attack = True
        else:
            if self.enemyhero.can_attack and self.enemyhero.present_damage > 0:
                can_attack = True
        return can_attack
    ...
    def follower_can_attack(self):
        ...
        if self.myturn:
            for card in self.myworldcards:
                if card.property.find('Rush') != -1 and not card.isRushed and card.can_attack:
                    if len(self.enemyworldcards) != 0:
                        can_attack_list.append(card)
                else:
                    if card.can_attack and card.sleep == False and card.present_damage > 0:
                        can_attack_list.append(card)
        else:
            for card in self.enemyworldcards:
                if card.property.find('Rush') != -1 and not card.isRushed and card.can_attack:
    ...
    def do_action(self):
        while self.can_use_skill() or self.hero_can_attack() or len(self.can_attack()) != 0 or len(
    ...
    def update_previousround_cards(self):
        ...
        if self.myturn:
            ...
            self.myhero.can_attack = True
        else:
            for card in self.enemyworldcards:
                if card.change_damage != 0:
                    card.back_to_present_damage()
                card.can_attack = True
            self.enemyhero.can_attack = True
    ...
    def battle(self):
        ...
        """
        if self.myturn:
            can_attack_list = self.can_attack()
            ...
            for i in range(len(self.enemyworldcards) - 1, -1, -1):
                ...
                if (self.enemyworldcards[i].present_damage > 0 and self.enemyworldcards[i].sleep == False) or (
                    ...
                    if opposite[rand_num].property != '':
                        ...
                        print('')
                    if self.enemyworldcards[i].property.find('Rush') != -1 and not self.enemyworldcards[i].isRushed:
                        self.enemyworldcards[i].attack(self.myworldcards[rand_num], self.enemyhero, self.myhero)
                        self.myworldcards[rand_num].show()
                    else:
                        if rand_num == 0:
                            self.enemyworldcards[i].attack(self.myhero, self.enemyhero)
                            self.myhero.show()
                        else:
                            self.enemyworldcards[i].attack(self.myworldcards[rand_num - 1], self.enemyhero,
                                                           self.myhero)
                            self.myworldcards[rand_num - 1].show()
