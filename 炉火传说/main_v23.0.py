...
def attack(attack_card, be_attacked_card, i, enemy_id):
    ...
    print("%s%s-%s attack %s%s-%s" % (
        attack_card[i].name, attack_card[i].present_damage,
        attack_card[i].remain_health,
        be_attacked_card[enemy_id].name, be_attacked_card[enemy_id].present_damage,
...
def choose_direct_num(direct_list, desc):
    ...
    if direct_num == 0:
        return direct_num
    elif card_has_stealth(direct_list):
        if direct_list[direct_num - 1].property.find('Stealth') == -1:
            return direct_num
        else:
            print("潜行的卡牌无法被选中！")
...
def choose_attack_num(attack_list, desc):
    ...
    elif card_has_stealth(attack_list):
...
class World:
    ...
    def show_world(self):
        ...
        if self.enemyhero.present_damage > 0:
            print("({0}-{1})".format(self.enemyhero.present_damage, self.enemyhero.weapon_durability))
        else:
            print("")
        if len(self.enemyworldcards) > 0:
            for card in self.enemyworldcards:
                print("{0}({1}-{2}/{3})".format(card.name, card.present_damage, card.remain_health, card.health),
                      end='')
                if card.property != '':
        ...
        if self.myhero.present_damage > 0:
            print("({0}-{1})".format(self.myhero.present_damage, self.myhero.weapon_durability))
        else:
            print("")
    ...
    def battle_cry(self, card, myhero, oppohero, myworldcards, oppoworldcards, myhandcards, oppohandcards):
        if card.name == '叫嚣的中士':
            if len(myworldcards) + len(oppoworldcards) == 0:
                pass
            else:
                if self.myturn:
                    world = myworldcards + oppoworldcards
                else:
                    world = myworldcards
                for i in range(len(world)):
                    print("{0}-{1}({2}-{3}".format(i + 1, world[i].name, world[i].present_damage,
            ...
            if len(myhandcards) < 10:
                myhandcards.append(Card('', '烈焰元素', 'element', '', 1, 2, 1))
                    ...
                    if goto_num == 0:
                        ...
                        if self.myhandcards[goto_num - 1].property.find('Battlecry') != -1:
                            self.battle_cry(self.myhandcards[goto_num - 1], self.myhero, self.enemyhero,
                                            self.myworldcards,
                                            self.enemyworldcards, self.myhandcards, self.enemyhandcards)
        ...
        while index != -2:
            ...
            if index != -2:
                ...
                if self.enemyhandcards[index].property.find('Battlecry') != -1:
                    self.battle_cry(self.enemyhandcards[index], self.enemyhero, self.myhero, self.enemyworldcards,
                                    self.myworldcards, self.enemyhandcards, self.myhandcards)
