...
class World:
    ...
    def skill_using(self, hero, enemy_hero=None):
        if hero.hero_class == 'warrior':
            ...
            print('')
            choose_num = choose_object_num(world)
    ...
    def battle(self):
        if self.myturn:
            ...
            for i in range(len(mysite) - 1, -1, -1):
                # 如果是我方英雄且武器有耐久
                if i == 0 and self.myhero.weapon_durability > 0:
                    print("------------攻击----------------")
                    print("{0}({1}[{2}]-{3}) 准备攻击：".format(self.myhero.name, self.myhero.damage,
                                                           self.myhero.weapon_durability,
                                                           self.myhero.remain_health))
            ...
            for i in range(len(enemysite) - 1, -1, -1):
                # 如果是敌方英雄且武器有耐久
                if i == 0 and self.enemyhero.weapon_durability > 0:
