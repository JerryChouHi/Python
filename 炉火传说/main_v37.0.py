...
class World:
                ...
                if len(self.myhandcards) >= HANDCARDNUM:
                    ...
                    if draw_card.name == '堕落之血':
                        ...
                        while BloodOfFallenNum > 0:
                            ...
                            self.mycards.insert(randnum1, parse_card(get_card_info(96).strip('\n')))
                            randnum2 = randint(0, len(self.mycards))
                            self.mycards.insert(randnum2, parse_card(get_card_info(96).strip('\n')))
                            BloodOfFallenNum -= 1
            else:
                self.myhero.tired += 1
                ...
                if len(self.enemyhandcards) >= HANDCARDNUM:
                    ...
                    if draw_card.name == '堕落之血':
                        ...
                        while BloodOfFallenNum > 0:
                            ...
                            self.enemycards.insert(randnum1, parse_card(get_card_info(96).strip('\n')))
                            randnum2 = randint(0, len(self.enemycards))
                            self.enemycards.insert(randnum2, parse_card(get_card_info(96).strip('\n')))
                            BloodOfFallenNum -= 1
            else:
    ...
    def skill_using(self):
        ...
        if self.myturn:
            ...
            if self.myhero.hero_class == 'Warrior':
                ...
                print("英雄技能：匕首精通")
                sword = parse_card(get_card_info(97).strip('\n'))
                if self.myweapon is not None:
                ...
                soldier = parse_card(get_card_info(98).strip('\n'))
                print("英雄技能：援军")
    ...
    def use_handcard(self):
        ...
        if self.myturn:
            ...
            if goto_num == 0:
                return
            elif self.myhandcards[goto_num - 1].present_mana <= self.current_mana:
                            ...
                            if len(self.myworldcards) == 0:
                                ...
                                if choose_num < len(self.myworldcards):
                                    if self.myworldcards[choose_num].race.find('Machine') != -1 and self.myhandcards[
                                        ...
                                        property_list = self.myhandcards[goto_num - 1].present_property.split(' ')
                ...
                print("输入卡牌的水晶({0})大于剩余水晶({1})，请重新输入！".format(self.myhandcards[goto_num - 1].present_mana,
                                                              self.current_mana))
        else:
    ...
    def death_rattle(self, card, myhero, oppohero, myworldcards, oppoworldcards, myhandcards, oppohandcards,
        ...
        if card.name == '腐烂的苹果树':
            ...
            if len(myhandcards) < HANDCARDNUM - 1:
                myhandcards.append(parse_card(get_card_info(99).strip('\n')))
                myhandcards.append(parse_card(get_card_info(99).strip('\n')))
            elif len(myhandcards) == HANDCARDNUM - 1:
                myhandcards.append(parse_card(get_card_info(99).strip('\n')))
        elif card.name == '紫色岩虫':
            while len(myworldcards) < WORLDCARDNUM:
                myworldcards.append(parse_card(get_card_info(100).strip('\n')))
            ...
            mylibrarycards.insert(myrandnum, parse_card(get_card_info(96).strip('\n')))
            opporandnum = randint(0, len(oppolibrarycards) - 1)
            oppolibrarycards.insert(opporandnum, parse_card(get_card_info(96).strip('\n')))
        elif card.name == '霜之哀伤':
            if self.myturn:
    ...
    def battle_cry(self, card, myhero, oppohero, myworldcards, oppoworldcards, myhandcards, oppohandcards,
        ...
        :return: 
        """
        self.current_mana -= card.present_mana
        if card.name == '死亡之翼':
            print("我就是力量的化身！！！")
            for worldcard in self.myworldcards:
            ...
            if card.name == '火羽精灵':
                if len(myhandcards) < HANDCARDNUM:
                    myhandcards.append(parse_card(get_card_info(101).strip('\n')))
            elif card.name == '穆克拉':
                banana = parse_card(get_card_info(102).strip('\n'))
                ...
                for i in range(len(choose_followers)):
                    print("{0}-{1}({2}-{3}".format(i + 1, choose_followers[i].name,
                ...
                if len(oppoworldcards) < WORLDCARDNUM - 1:
                    oppoworldcards.append(parse_card(get_card_info(103).strip('\n')))
                    oppoworldcards.append(parse_card(get_card_info(103).strip('\n')))
    ...
    def auto_usehandcard(self):
        ...
        if can_use_list[index].type == 'Follower':
            if can_use_list[index].present_property.find('Battlecry') != -1:
                self.battle_cry(can_use_list[index], self.enemyhero, self.myhero,
                                self.enemyworldcards, self.myworldcards, self.enemyhandcards,
                                self.myhandcards, self.enemycards, self.mycards)
            else:
                self.enemyworldcards.append(can_use_list[index])
                print(">>>>>>{0}({1})".format(can_use_list[index].name,
                                              can_use_list[index].present_mana),
                      end='')
                if can_use_list[index].present_property != '':
                    print("[{0}]".format(can_use_list[index].present_property), end='')
                print("上场！！！")
                self.current_mana -= can_use_list[index].present_mana
        elif can_use_list[index].type == 'Magic':
            if self.cast(can_use_list[index], self.enemyhero, self.myhero,
                         self.enemyworldcards, self.myworldcards, self.enemyhandcards, self.myhandcards,
                         self.enemycards, self.mycards):
                self.current_mana -= can_use_list[index].present_mana
            else:
                return
        elif can_use_list[index].type == 'Weapon':
            if self.enemyweapon is not None:
                self.enemyweapon.alive = False
                self.update_weapon()
            self.enemyweapon = can_use_list[index]
            print(">>>装备 {0}".format(self.enemyweapon.name))
            self.current_mana -= can_use_list[index].present_mana
        try:
            if can_use_list[index] in self.enemyhandcards:
