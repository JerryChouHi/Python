...
#             36.乌达斯塔：超杀OverKill--从你的手牌中召唤一个野兽
#                恩佐斯：战吼Battlecry--召唤所有你在本局对战中死亡的，并具有亡语的随从
#             37.伊瑟拉：在你的回合结束时,随机将一张梦境牌置入你的手牌
#                根据卡牌id从卡牌库中获取卡牌信息来生成卡牌对象
#             38.老狐狸马林：战吼Battlecry--为你的对手召唤一个0/8的宝箱。
#                大师宝箱：亡语Deathrattle--使你的对手获得一份惊人的战利品
...
class World:
    ...
    def card_can_use(self, card):
        ...
        if self.myturn:
            if card.type == 'Follower':
                if card.present_mana <= self.current_mana and len(self.myworldcards) < WORLDCARDNUM:
                    can_use = True
            elif card.type == 'Maigc' and card.isDirectMagic and card.present_mana <= self.current_mana:
                for worldcard in self.myworldcards:
            ...
            if card.type == 'Follower':
                if card.present_mana <= self.current_mana and len(self.enemyworldcards) < WORLDCARDNUM:
                    can_use = True
            elif card.type == 'Maigc' and card.isDirectMagic and card.present_mana <= self.current_mana:
                for worldcard in self.enemyworldcards:
                    if worldcard.present_property.find('AntiMagic') == -1:
    ...
    def card_can_attack(self, card):
        ...
        if self.myturn:
            if card.present_property.find('Rush') != -1:
                if card.present_attack_count > 0 and card.present_damage > 0 and card_has_notstealth(
                        self.enemyworldcards):
                    can_attack = True
            else:
                ...
                if len(self.myhandcards) >= HANDCARDNUM:
                    ...
                    if draw_card.name == '堕落之血':
                        ...
                        while BloodOfFallenNum > 0:
                            ...
                            self.mycards.insert(randnum1, parse_card(get_card_info(1001).strip('\n')))
                            randnum2 = randint(0, len(self.mycards))
                            self.mycards.insert(randnum2, parse_card(get_card_info(1001).strip('\n')))
                            BloodOfFallenNum -= 1
            else:
                self.myhero.tired += 1
                ...
                if len(self.enemyhandcards) >= HANDCARDNUM:
                    ...
                    if draw_card.name == '堕落之血':
                        ...
                        while BloodOfFallenNum > 0:
                            ...
                            self.enemycards.insert(randnum1, parse_card(get_card_info(1001).strip('\n')))
                            randnum2 = randint(0, len(self.enemycards))
                            self.enemycards.insert(randnum2, parse_card(get_card_info(1001).strip('\n')))
                            BloodOfFallenNum -= 1
            else:
    ...
    def skill_using(self):
        ...
        if self.myturn:
            ...
            if self.myhero.hero_class == 'Warrior':
                ...
                print("英雄技能：匕首精通")
                sword = parse_card(get_card_info(1002).strip('\n'))
                if self.myweapon is not None:
                ...
                soldier = parse_card(get_card_info(1003).strip('\n'))
                print("英雄技能：援军")
    ...
    def use_handcard(self):
        ...
        if self.myturn:
            ...
            if goto_num == 0:
                return
            if self.card_can_use(self.myhandcards[goto_num - 1]):
                ...
                if temp_handcard.type == 'Follower':
                    ...
                    self.cast(temp_handcard, self.myhero, self.enemyhero,
                              self.myworldcards, self.enemyworldcards, self.myhandcards,
                              self.enemyhandcards, self.mycards, self.enemycards)
    ...
    def hero_battle(self):
        ...
        if self.myturn:
            ...
            print(
                "{0}{1}-{2} ".format(self.enemyhero.name, self.enemyhero.present_damage, self.enemyhero.remain_health),
                end='')
            if self.enemyhero.armor > 0:
                print("[{0}]".format(self.enemyhero.armor), end=' ')
    ...
    def follower_battle(self):
        ...
        if self.myturn:
            for i in range(len(self.myworldcards)):
                ...
                if self.card_can_attack(self.myworldcards[i]):
                    print("<Active>",end=' ')
                else:
                    print("<No Active>",end=' ')
            print('')
            ...
            if ready_attack_num == 0:
                return
            if self.card_can_attack(self.myworldcards[ready_attack_num - 1]):
                if self.myworldcards[ready_attack_num - 1].present_property.find('Rush') != -1:
                ...
                if self.enemyhero.remain_health <= 0:
                    return
                if self.myworldcards[ready_attack_num - 1].name == '凶恶的雏龙' and opposite[attack_num - 1].type == 'Hero':
                    ...
                    if choose_list[choose - 1] == '+3攻':
                        print("{0} 的攻击力 {1} >>>".format(self.myworldcards[ready_attack_num - 1].name,
                                                        self.myworldcards[ready_attack_num - 1].present_damage), end='')
                        self.myworldcards[ready_attack_num - 1].buff_damage += 3
                        self.myworldcards[ready_attack_num - 1].update_buff_damage()
                        print("{0}".format(self.myworldcards[ready_attack_num - 1].present_damage))
                    elif choose_list[choose - 1] == '+3血':
                        print("{0} 生命值 {1}/{2} >>>".format(self.myworldcards[ready_attack_num - 1].name,
                                                           self.myworldcards[ready_attack_num - 1].remain_health,
                                                           self.myworldcards[ready_attack_num - 1].present_health),
                              end='')
                        self.myworldcards[ready_attack_num - 1].remain_health += 3
                        self.myworldcards[ready_attack_num - 1].present_health += 3
                        print("{0}/{1}".format(self.myworldcards[ready_attack_num - 1].remain_health,
                                               self.myworldcards[ready_attack_num - 1].present_health))
                    elif choose_list[choose - 1] == '+1攻+1血':
                        print("{0} {1}-{2}/{3} >>>".format(self.myworldcards[ready_attack_num - 1].name,
                                                           self.myworldcards[ready_attack_num - 1].present_damage,
                                                           self.myworldcards[ready_attack_num - 1].remain_health,
                                                           self.myworldcards[ready_attack_num - 1].present_health),
                              end='')
                        self.myworldcards[ready_attack_num - 1].buff_damage += 1
                        self.myworldcards[ready_attack_num - 1].update_buff_damage()
                        self.myworldcards[ready_attack_num - 1].remain_health += 1
                        self.myworldcards[ready_attack_num - 1].present_health += 1
                        print("{0}-{1}/{2}".format(self.myworldcards[ready_attack_num - 1].present_damage,
                                                   self.myworldcards[ready_attack_num - 1].remain_health,
                                                   self.myworldcards[ready_attack_num - 1].present_health))
                    elif choose_list[choose - 1] == '风怒':
                        if self.myworldcards[ready_attack_num - 1].present_property.find('WindAngry') == -1:
                            print("{0} 获得 风怒".format(self.myworldcards[ready_attack_num - 1].name))
                            self.myworldcards[ready_attack_num - 1].present_property += ' WindAngry'
                            self.myworldcards[ready_attack_num - 1].present_attack_count += 1
                            self.myworldcards[ready_attack_num - 1].attack_count = 2
                    elif choose_list[choose - 1] == '潜行':
                        if self.myworldcards[ready_attack_num - 1].present_property.find('Stealth') == -1:
                            print("{0} 获得 潜行".format(self.myworldcards[ready_attack_num - 1].name))
                            self.myworldcards[ready_attack_num - 1].present_property += ' Stealth'
                    elif choose_list[choose - 1] == '嘲讽':
                        if self.myworldcards[ready_attack_num - 1].present_property.find('Taunt') == -1:
                            print("{0} 获得 嘲讽".format(self.myworldcards[ready_attack_num - 1].name))
                            self.myworldcards[ready_attack_num - 1].present_property += ' Taunt'
            ...
            random_follower = random.choice(can_attack_list)
            if random_follower.present_property.find('Rush') != -1:
                opposite = self.myworldcards
            else:
                opposite = [self.myhero] + self.myworldcards
            print("------------攻击----------------")
            rand_num = self.attack_who(opposite)
            print("{0}{1}-{2} ".format(random_follower.name, random_follower.present_damage,
                                       random_follower.remain_health), end='')
            if random_follower.armor > 0:
                print("[{0}]".format(random_follower.armor), end='')
            if random_follower.present_property != '':
                print("[{0}]".format(random_follower.present_property), end='')
            else:
                print(end='')
            print(" attack {0}{1}-{2}".format(opposite[rand_num].name, opposite[rand_num].present_damage,
                                              opposite[rand_num].remain_health), end='')
            ...
            if opposite[rand_num].present_property != '':
                ...
                print('')
            if random_follower.attack(opposite[rand_num], self.enemyhero, self.myhero) and \
                            random_follower.present_property.find('OverKill') != -1:
                self.over_kill(random_follower, self.enemyhero, self.myhero, self.enemyworldcards, self.myworldcards,
                               self.enemyhandcards, self.myhandcards, self.enemycards, self.mycards)
            opposite[rand_num].show()
            if self.myhero.remain_health <= 0:
                return
            if random_follower.name == '凶恶的雏龙' and opposite[rand_num].type == 'Hero':
                choose = randint(1, 6)
                print(choose)
                if choose == 1:
                    print("{0} 的攻击力 {1} >>>".format(random_follower.name,
                                                    random_follower.present_damage),
                          end='')
                    random_follower.buff_damage += 3
                    random_follower.update_buff_damage()
                    print("{0}".format(random_follower.present_damage))
                elif choose == 2:
                    print("{0} 生命值 {1}/{2} >>>".format(random_follower.name,
                                                       random_follower.remain_health,
                                                       random_follower.present_health),
                          end='')
                    random_follower.remain_health += 3
                    random_follower.present_health += 3
                    print("{0}/{1}".format(random_follower.remain_health,
                                           random_follower.present_health))
                elif choose == 3:
                    print("{0} {1}-{2}/{3} >>>".format(random_follower.name,
                                                       random_follower.present_damage,
                                                       random_follower.remain_health,
                                                       random_follower.present_health),
                          end='')
                    random_follower.buff_damage += 1
                    random_follower.update_buff_damage()
                    random_follower.remain_health += 1
                    random_follower.present_health += 1
                    print("{0}-{1}/{2}".format(random_follower.present_damage,
                                               random_follower.remain_health,
                                               random_follower.present_health))
                elif choose == 4:
                    if random_follower.present_property.find('WindAngry') == -1:
                        print("{0} 获得 风怒".format(random_follower.name))
                        random_follower.present_property += ' WindAngry'
                        random_follower.present_attack_count += 1
                        random_follower.attack_count = 2
                elif choose == 5:
                    if random_follower.present_property.find('Stealth') == -1:
                        print("{0} 获得 潜行".format(random_follower.name))
                        random_follower.present_property += ' Stealth'
                elif choose == 6:
                    if random_follower.present_property.find('Taunt') == -1:
                        print("{0} 获得 嘲讽".format(random_follower.name))
                        random_follower.present_property += ' Taunt'
            random_follower.show()
    ...
    def over_kill(self, card, myhero, oppohero, myworldcards, oppoworldcards, myhandcards, oppohandcards,
        ...
        if card.name == '阵线破坏者':
            ...
            beast_list = Beast_cards(myhandcards)
            if len(beast_list) > 0:
    ...
    def death_rattle(self, card, myhero, oppohero, myworldcards, oppoworldcards, myhandcards, oppohandcards,
        ...
        if card.name == '腐烂的苹果树':
            ...
            if len(myhandcards) < HANDCARDNUM - 1:
                myhandcards.append(parse_card(get_card_info(1004).strip('\n')))
                myhandcards.append(parse_card(get_card_info(1004).strip('\n')))
            elif len(myhandcards) == HANDCARDNUM - 1:
                myhandcards.append(parse_card(get_card_info(1004).strip('\n')))
        elif card.name == '紫色岩虫':
            while len(myworldcards) < WORLDCARDNUM:
                myworldcards.append(parse_card(get_card_info(1005).strip('\n')))
            ...
            mylibrarycards.insert(myrandnum, parse_card(get_card_info(1001).strip('\n')))
            opporandnum = randint(0, len(oppolibrarycards) - 1)
            oppolibrarycards.insert(opporandnum, parse_card(get_card_info(1001).strip('\n')))
        elif card.name == '霜之哀伤':
            if self.myturn:
        ...
        if card.name == '死亡之翼':
            ...
            if card.name == '火羽精灵':
                if len(myhandcards) < HANDCARDNUM:
                    myhandcards.append(parse_card(get_card_info(1006).strip('\n')))
            elif card.name == '穆克拉':
                banana = parse_card(get_card_info(1007).strip('\n'))
                ...
                if len(oppoworldcards) < WORLDCARDNUM - 1:
                    oppoworldcards.append(parse_card(get_card_info(1008).strip('\n')))
                    oppoworldcards.append(parse_card(get_card_info(1008).strip('\n')))
                ...
                if self.myturn:
                    ...
                    for id in self.my_death_follower_id:
                        self.mydeathfollower.append(parse_card(get_card_info(id).strip('\n')))
                    deathrattle_list = Deathrattle_cards(self.mydeathfollower)
                    ...
                    for id in self.enemy_death_follower_id:
                        self.enemydeathfollower.append(parse_card(get_card_info(id).strip('\n')))
                    deathrattle_list = Deathrattle_cards(self.enemydeathfollower)
                    for deathcard in deathrattle_list:
                        if len(self.enemyworldcards) < WORLDCARDNUM:
                            print(">>>{0} -> {1}".format(card.name, deathcard.name))
                ...
                if len(oppoworldcards) < WORLDCARDNUM:
                    master_treasure_chest = parse_card(get_card_info(1026).strip('\n'))
                ...
                myhandcards.clear()
                while card_num > 0:
                    legend_card = random.sample(legend_cards(), 1)[0]
    ...
    def cast(self, card, myhero, oppohero, myworldcards, oppoworldcards, myhandcards, oppohandcards,
        ...
        if card.name == '香蕉':
            ...
            if self.myturn:
                ...
                for worldcard in self.myworldcards:
                    if worldcard.present_property.find('AntiMagic') == -1 and worldcard.present_property.find(
                            'Stealth') == -1:
                        opposite.append(worldcard)
            if len(mysite) > 0 and len(opposite) > 0:
                if self.myturn:
                    print("1.我方角色 2.敌方角色")
                    choose_charater = input_int("请选择角色的序号：")
                else:
                    choose_charater = randint(1, 2)
            elif len(mysite) == 0 and len(opposite) > 0:
                choose_charater = 2
            elif len(mysite) > 0 and len(opposite) == 0:
                choose_charater = 1
            if choose_charater == 1:
                ...
                if self.myturn:
                    ...
                    choose_num = randint(1, len(mysite))
                print(">>>{0}->{1} 从战场移回手牌".format(card.name, mysite[choose_num - 1].name))
                mysite[choose_num - 1].reset()
                myhandcards.append(mysite[choose_num - 1])
                myworldcards.remove(mysite[choose_num - 1])
            else:
                for i in range(len(opposite)):
                    print("{0}-{1}({2}-{3}".format(i + 1, opposite[i].name, opposite[i].present_damage,
                                                   opposite[i].remain_health), end='')
                ...
                if self.myturn:
                    ...
                    choose_num = randint(1, len(opposite))
                print(">>>{0}->{1} 从战场移回手牌".format(card.name, opposite[choose_num - 1].name))
                opposite[choose_num - 1].reset()
                oppohandcards.append(opposite[choose_num - 1])
                oppoworldcards.remove(opposite[choose_num - 1])
        elif card.name == '梦魇':
            if self.myturn:
                world = []
                for worldcard in self.myworldcards:
            ...
            if self.myturn:
                ...
                choose_num = randint(1, len(world))
            print(">>>{0}->{1} {2}-{3}/{4}->".format(card.name, world[choose_num - 1].name,
                                                     world[choose_num - 1].present_damage,
                                                     world[choose_num - 1].remain_health,
                                                     world[choose_num - 1].present_health), end='')
            world[choose_num - 1].buff_damage += 5
            world[choose_num - 1].update_buff_damage()
            world[choose_num - 1].remain_health += 5
            world[choose_num - 1].present_health += 5
            print("{0}-{1}/{2}".format(world[choose_num - 1].present_damage,
                                       world[choose_num - 1].remain_health,
                                       world[choose_num - 1].present_health))
            if self.myturn:
                if world[choose_num - 1].present_property == '':
                    world[choose_num - 1].present_property = 'MyNightmare'
                else:
                    world[choose_num - 1].present_property += ' MyNightmare'
            else:
                if world[choose_num - 1].present_property == '':
                    world[choose_num - 1].present_property = 'EnemyNightmare'
                else:
                    world[choose_num - 1].present_property += ' EnemyNightmare'
            ...
            choose_followers = random.sample(legend_cards(), 3)
            for i in range(len(choose_followers)):
                print("{0}-{1}({2}-{3}/{0})".format(i + 1, choose_followers[i].name,
                                                    choose_followers[i].present_damage,
                                                    choose_followers[i].remain_health,
            ...
            while count > 0:
                if len(myworldcards) < WORLDCARDNUM:
                    myworldcards.append(parse_card(get_card_info(choose_followers[choose_num - 1].id).strip('\n')))
                count -= 1
    ...
    def auto_usehandcard(self):
        ...
        :return: 
        """
        can_use_list = self.can_use_handcard()
        ...
        if temp_handcard.type == 'Follower':
            ...
            self.cast(temp_handcard, self.enemyhero, self.myhero,
    ...
    def attack_who(self, cards):
        ...
        if len(cards) == 1:
            ...
            if card_has_taunt(cards, False):
                if cards[card_id].present_property.find('Taunt') != -1 and cards[card_id].present_property.find(
                        'Stealth') == -1:
                    return card_id
                else:
