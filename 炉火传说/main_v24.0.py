    ...
    def battle_cry(self, card, myhero, oppohero, myworldcards, oppoworldcards, myhandcards, oppohandcards):
        if card.name == '叫嚣的中士':
            ...
            if len(world) == 0:
                ...
                print('')
                if self.myturn:
                    choose_num = choose_object_num(world, "请选择对象序号：")
                else:
                    choose_num = randint(1, len(world))
            ...
            if len(oppohandcards) < 9:
                oppohandcards.append(banana)
                oppohandcards.append(banana)
            elif len(oppohandcards) == 9:
                oppohandcards.append(banana)
        ...
        if card.name == '香蕉':
            ...
            if len(world) == 0:
                ...
                if self.myturn:
                    ...
                    while True:
                        choose_num = randint(1, len(world))
                        if not card_has_AntiMagic(world[choose_num - 1]):
                            break
            ...
            while self.can_use_skill(mana) or (
                ...
                if enough_mana(self.myhandcards, mana) and len(self.myworldcards) < WORLDCARDNUM:
                    ...
                    if goto_num == 0:
                        ...
                        if self.myhandcards[goto_num - 1].type == 'Magic':
    ...
    def auto_goto_world(self, mana):
        ...
        while index != -2:
            ...
            if index != -2:
                ...
                if self.enemyhandcards[index].type == 'Magic':
                    if not self.cast(self.enemyhandcards[index], self.enemyhero, self.myhero, self.enemyworldcards,
                                     self.myworldcards, self.enemyhandcards, self.myhandcards):
                        break
                else:
