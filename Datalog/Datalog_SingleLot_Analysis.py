        ...
        for col_num in range(last_test_item_col_num + 1):
            ...
            if col_offset <= col_num <= binning_col_num and high_limit_row_num <= row_num <= low_limit_row_num:
                ...
                try:
                    ...
                    if test_item_name == 'AVDD_O/S' or high_limit == 'N' or (
...
def save_data(analysis_folder, parse_data):
    ...
    for data in parse_data:
        ...
        for i in range(len(datacheck_data)):
            ...
            #     for m in range(len(data_check[0])):
            #         data_check_sheet.cell(row=irow, column=m + 1).value = data_check[0][m]
            #         data_check_sheet.cell(row=irow, column=m + 1).fill = PatternFill(fill_type='solid',
            #                                                                           fgColor=data_check_color)
            #         data_check_sheet.cell(row=irow, column=m + 1).border = border
            #         if m == len(data_check[0]) - 1:
            #             for x in range(m + 1, len(datacheck_data[0])):
            #                 data_check_sheet.cell(row=irow, column=x + 1).border = border
            #     irow += 1
            #     for n in range(len(data_check[1])):
            #         data_check_sheet.cell(row=irow, column=n + 1).value = data_check[1][n]
            #         data_check_sheet.cell(row=irow, column=n + 1).fill = PatternFill(fill_type='solid',
            #                                                                           fgColor=data_check_color)
            #         data_check_sheet.cell(row=irow, column=n + 1).border = border
            #         if n == len(data_check[0]) - 1:
            #             for x in range(n + 1, len(datacheck_data[0])):
            #                 data_check_sheet.cell(row=irow, column=x + 1).border = border
            #     irow += 1
...
def main():
    ...
    # get project's data check
    # for i in range(len(data_check_list[project_id])):
    #     data_check[0].append(data_check_list[project_id][i][1])
    #     data_check[1].append(data_check_list[project_id][i][2])
